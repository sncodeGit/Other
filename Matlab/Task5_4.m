...
function [] = main()
    ...
    % численного интегрирования для явных методов численного интегрирования
    % К сожалению, вероятно в данном случае шаг явного метода должен быть куда меньше
...
function next_val = euler_implicit_step(x, y, z, delta)
    ...
    % =>
    % y_(i+1) = y_i + delta * z_(i+1)
    % z_(i+1) = z_i - 100000.001 * delta * z_(i+1) - 100 * delta * y_(i+1)
    % =>
    % y_(i+1) = y_i + delta * z_(i+1)
    % z_(i+1) = z_i - 100000.001 * delta * z_(i+1) - 100 * delta * (y_i + delta * z_(i+1))
    % =>
    % y_(i+1) = y_i + delta * z_(i+1)
    % z_(i+1) = z_i - 100000.001 * delta * z_(i+1) - 100 * delta * y_i - 100 * delta^2 * z_(i+1)
    % =>
    % y_(i+1) = y_i + delta * z_(i+1)
    % z_(i+1) * (1 + 100000.001 * delta + 100 * delta^2) = z_i - 100 * delta * y_i
    % =>
    % y_(i+1) = y_i + delta * z_(i+1)
    % z_(i+1) = (z_i - 100 * delta * y_i) / (1 + 100000.001 * delta + 100 * delta^2)
    next_z = (z - 100 * delta * y) / (1 + 100000.001 * delta + 100 * delta * delta);
